        ...
        var _mainVue = new Vue({
            ...
            beforeMount: function() {
                ...
                if (_hash[0].indexOf('create') != -1) {
                    ...
                    this.form.user.email = "sample@test.pl";
                } else {
                    if (!this.user) {
                        location.href = './#/login';
                        this.user = {
                    ...
                    if (this.price) {
                        ...
                        this.$http.get('/api/menu-prices/findOne?filter[where][year]=' + _current.year + '&filter[where][week][lt]=' + _current.week + '&filter[order]=created_at DESC').then(
                            function(sucess) {
                                // console.log(sucess);
            ...
            methods: {
                chengeDiner: function(id) {
                    ...
                    this.$http.get('/api/diners/' + id + '/orders?filter[where][year]=' + _current.year + '&filter[where][week]=' + _current.week + '&filter[order]=created_at DESC').then(
                        function(sucess) {
                            // console.log(sucess);
                            for (var i = 1; i < 6; i++) {
                                if (this.form.day[i][0].indexOf('disabled') != -1) {
                                    this.form.day[i][0] = ' disabled ';
                                    this.form.day[i][1] = ' disabled ';
                                    this.form.day[i][2] = ' disabled ';
                                    this.form.day[i][3] = ' disabled ';
                                } else {
                                    this.form.day[i][0] = this.form.day[i][1] = this.form.day[i][2] = this.form.day[i][3] = '';
                                }
                            }
                            for (var i = 1; i < 6; i++) {
                                this.orderSet[i] =
                                    this.form.order.set[i] = [0, 0, 0, 0];
                            }
                            //
                            var len = sucess.body.length;
                            if (sucess.body.length) {
                                this.orderSet = this.form.set = sucess.body[0].set;
                                for (var i = 1; i < 6; i++) {
                                    var _all = false;
                ...
                buy: function(e) {
                    ...
                    //
                    var _ect = e.currentTarget;
                    _ect.disabled = 'disabled';
                    //
                    // console.log('order: ', JSON.stringify(this.form.order.set));
                    //console.log('order: ', JSON.stringify(this.orderSet));
                    var _order = this.orderSet,
                        _set = this.form.order.set;
                    this.form.order.set.map(function(item, index) {
                        if (index != 0) {
                            var order = _order[index];
                            item = item.map(function(itemBis, indexBis) {
                                if (itemBis == -1) itemBis = 0;
                                else if (order[indexBis]) itemBis = 1;
                                return itemBis;
                            });
                        }
                        _set[index] = item;
                    ...
                    this.$http.patch('/api/users/' + this.user.id + '?access_token=' + this.user.accessToken, '{\"wallet\":' + this.user.data.wallet + '}').then(
                        function(sucess) {
                            this.$http.post('/api/orders?access_token=' + this.user.accessToken, JSON.stringify(this.form.order)).then(
                                function(sucess) {
                                    //this.form.order = 
